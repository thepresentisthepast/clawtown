...
async function loadMemo() {
  ...
  try {
    const response = await fetch('/api/star-office/yesterday-memo?t=' + Date.now(), { cache: 'no-store' });
...
function setState(state, detail) {
  fetch('/api/star-office/set-state', {
    method: 'POST',
...
function preload() {
  ...
  this.load.image('office_bg', '/star-office/office_bg_small' + (supportsWebP ? '.webp' : '.png') + '?v=20240302');
...
function fetchAgents() {
  fetch('/api/star-office/agents?t=' + Date.now(), { cache: 'no-store' })
    .then(response => response.json())
    .then(data => {
